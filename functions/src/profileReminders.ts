...
function generateContentBlock(lang: 'es' | 'en' | 'de', clientName: string, missingKeys: FieldKey[], editUrl: string, unsubscribeUrl: string): string {
    const texts = {
        es: {
    ...
    try {
        ...
        for (let i = 0; i < allDocs.length; i += BATCH_SIZE) {
            ...
            if (lang === 'all') {
                ...
                if (lang === 'es') subject = '¡Tu perfil en Dicilo está casi listo! 🚀';
                if (lang === 'en') subject = 'Your Dicilo profile is almost ready! 🚀';
                if (lang === 'de') subject = 'Dein Dicilo-Profil ist fast fertig! 🚀';
