...
export function generateFreelancerPDF(
    referrerName: string,
    referrerCode: string,
    referrerRole: string,
    periodStr: string,
...
export function generateAdminPDF(
    periodStr: string,
    registrations: any[],
    summary: {
        ...
        platformTotalDP: number;
        platformTotalCash: number;
    }
): Buffer {
    const doc = new jsPDF();
...
export async function runMonthlyReport(year: number, month: number, isTest: boolean, testEmailOverride?: string) {
    const db = admin.firestore();
    ...
    const allProfiles = profilesSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })) as any[];
...
export const sendMonthlyReferralReports = onSchedule({
    ...
    try {
        await runMonthlyReport(targetYear, targetMonth, false);
