        ...
        <div className="p-8 max-w-6xl mx-auto space-y-8">
            <div className="pb-4 border-b border-slate-200">
                <h1 className="text-3xl font-bold tracking-tight text-slate-900">
                    <span dangerouslySetInnerHTML={{ __html: t('business.financials.title', 'Wallet <span class="text-blue-600">Empresarial</span>') }}></span>
                </h1>
                <p className="text-slate-500 mt-2 text-lg">{t('business.financials.desc', 'Consulta y gestiona tus saldo en puntos y comisiones disponibles.')}</p>
            </div>
                ...
                <Card className="border-slate-200 shadow-lg relative overflow-hidden bg-slate-900 text-white">
                    ...
                    <CardHeader className="pb-2">
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-xl font-medium tracking-wide flex items-center gap-2">
                                ...
                                {t('business.financials.dpTitle', 'Dicilo Puntos (DP)')}
                            </CardTitle>
                            <span className="bg-slate-800 text-slate-300 text-xs px-2 py-1 rounded-md font-mono border border-slate-700">{t('business.financials.accumulated', 'Acumulados')}</span>
                        </div>
                        <CardDescription className="text-slate-400">{t('business.financials.dpDesc', 'Puntos recibidos de los usuarios como descuento.')}</CardDescription>
                    </CardHeader>
                    <CardContent className="pt-6 relative z-10">
                        ...
                        <p className="text-sm text-blue-300 mt-2 flex items-center gap-1.5 opacity-90">
                            ...
                            {t('business.financials.dpEquivalent', `Equivale a ${dpInEuros.toFixed(2)}€ para pago de publicidad B2B.`).replace('${EUR}', dpInEuros.toFixed(2))}
                        </p>
                        ...
                        <div className="mt-8 pt-6 border-t border-slate-700/50 flex gap-3">
                            <Button disabled={plan === 'basic' || balances.dp === 0} className="bg-blue-600 hover:bg-blue-500 text-white flex-1" onClick={() => alert("Módulo Ad-Manager conectando...")}>
                                {t('business.financials.payAdsBtn', 'Pagar Anuncios con DP')}
                            </Button>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardHeader className="pb-2">
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-xl font-medium tracking-wide text-teal-900 flex items-center gap-2">
                                ...
                                {t('business.financials.eurTitle', 'Comisiones Efectivas')}
                            </CardTitle>
                            <span className="bg-white text-teal-700 text-xs px-2 py-1 rounded-md font-mono border border-teal-200">Cashback B2B</span>
                        </div>
                        <CardDescription className="text-teal-700/70">{t('business.financials.eurDesc', 'Saldo generado por referir otras empresas a Dicilo.')}</CardDescription>
                    </CardHeader>
                    <CardContent className="pt-6 relative z-10">
                        ...
                        <p className="text-sm text-teal-600/80 mt-2 flex items-center gap-1.5 font-medium">
                            {t('business.financials.eurAvailable', 'Disponible de inmediato para retiro o facturación.')}
                        </p>
            ...
            {(plan === 'basic') && (
                <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-lg flex items-start gap-4 text-sm font-medium mt-6">
                    ...
                    <p>{t('business.financials.basicWarning', `Tu plan ${plan} solo te permite observar el saldo. Necesitas mejorar al menos a Starter para recibir comisiones B2B operativas.`).replace('${plan}', plan)}</p>
                </div>
            )}
        </div>
    );
