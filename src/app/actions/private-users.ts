...
export async function setPrivateUserRole(uid: string, role: string) {
    const validRoles = ['user', 'freelancer', 'team_leader', 'team_office', 'admin', 'superadmin', 'apoyo_social'];
    ...
    try {
        await getAdminDb().collection('private_profiles').doc(uid).update({
            role: role,
            isFreelancer: role === 'freelancer' || role === 'team_leader' || role === 'team_office' || role === 'admin' || role === 'superadmin', // Backwards compatibility
            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
